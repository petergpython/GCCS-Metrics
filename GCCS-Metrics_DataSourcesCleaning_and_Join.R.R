...
### Data sources cleaning individually and Join into a single dataset
#### Set working directory ####
setwd("path_working_directory_here")
...
#library(write.csv)
## 5 Data sources into one dataset
# (1). BGCI Plant Search
# (2). FAO WIEWS
# (3). GBIF (Dropped)
# (4). Genesys PGR
...
BGCI_allcrops <- read_excel("data_sources/BGCIPlantSearch_data/BGCI_allcrops_unformatted.xlsx")
WIEWS_allcrops <- read_csv("data_sources/FAOWIEWS_data/SDGBrequestExp.csv")
SGSV_allcrops <- read_excel("data_sources/SGSV_data/SGSV_allcrops_unformatted.xlsx")
Genesys_allcrops <- read_csv("data_sources/GenesysPGR_data/Genesys_allcrops_unformatted.csv")# Read in as a csv, not excel, helped eliminate data loss
GBIF_allcrops <- read_csv("data_sources/GBIF_data/GBIF_allcrops_unformatted.csv")
...
geo_names <- read_csv("data_sources/processing/geo_names.csv" , na = c("", "-"))
...
institute_names <- read_excel("data_sources/processing/FAO_WIEWS_organizations_PG_with_synonyms.xlsx")
...
institute_names_no_syn <- read_excel("data_sources/processing/FAO_WIEWS_organizations_PG.xlsx")
...
BGCI_allcrops <- BGCI_allcrops %>%
  mutate(fullTaxa = ifelse(is.na(fullTaxa) | fullTaxa == "", fullTaxa, SubmittedName))
...
correct_duplsite_codes <- read.csv("data_sources/processing/manual_entry_duplsite.csv")
...
#Genesys_allcrops <- read_csv("data_sources/GenesysPGR_data/Genesys_allcrops_unformatted.csv")
## fields to keep: 
...
# add organization type (CGIAR, International, Governmental)
organizations_type <- setNames(institute_names_no_syn$ORGANIZATIONTYPE, institute_names_no_syn$INSTCODE)
combined_df2$ORGANIZATIONCATEGORY = NA
combined_df2 <- combined_df2 %>%
  mutate(ORGANIZATIONCATEGORY = ifelse(is.na(ORGANIZATIONCATEGORY), organizations_type[INSTCODE], ORGANIZATIONCATEGORY))
# manually correct organization type for TWN001
combined_df2 <- combined_df2 %>%
  mutate(ORGANIZATIONCATEGORY = ifelse(INSTCODE == 'TWN001', 'International', ORGANIZATIONCATEGORY))

######################## Add Crop_strategy categorical variable ############################################
# Read the Excel file
crops <- read_excel("data_sources/processing/croplist_PG.xlsx")
...
combined_df2$Crop_strategy = NA
combined_df2 <- combined_df2 %>%
  mutate(Crop_strategy = ifelse(is.na(Crop_strategy), crop_strategy_dict[GENUS], Crop_strategy))


## Recode SAMPSTAT as isCrop
# Replace NA values in the 'SAMPSTAT' column with 0
combined_df2$SAMPSTAT <- ifelse(is.na(combined_df2$SAMPSTAT), 0, combined_df2$SAMPSTAT)

# Convert 'SAMPSTAT' to integer, then to character
combined_df2$SAMPSTAT <- as.character(as.integer(combined_df2$SAMPSTAT))

# Define the crop codes
crop_codes <- c('300', '400', '410', '411', '412', '413', '414', '415', '416', '417', '418', '419', '420', '421', '422', '423', '500', '600')
                
# Update 'SAMPSTAT' based on conditions
combined_df2$isCrop <- ifelse(combined_df2$SAMPSTAT %in% c('100', '110', '120', '130'), 'Wild', combined_df2$SAMPSTAT)
combined_df2$isCrop <- ifelse(combined_df2$isCrop %in% c('200'), 'Weedy', combined_df2$isCrop)
combined_df2$isCrop <- ifelse(combined_df2$isCrop %in% crop_codes, 'Crop', combined_df2$isCrop)
# To be completed and discussed with Colin
write_csv(combined_df2, "combined07_05_25.csv")
